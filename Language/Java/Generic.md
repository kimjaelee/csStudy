# Generic
> 파라미터 타입이나 리턴 타입에 대한 정의를 외부로 미룸<br>
>타입에 대해 유연성과 안정성을 확보<br>
>**런타임 환경에 아무런 영향이 없는 컴파일 시점의 전처리 기술**<br>
> 타입을 유연하게 처리, 런타임에 발생할 수 있는 타입 에러를 컴파일 전에 검출할 수 있다.<br>
...
```java
	@Testpublic void test(){List<String> example = new ArrayList<>();
...
1. **제네릭 타입 정의시 상속관계를 명시하는 방법** (와일드카드를 사용한다)
	...
	@FunctionalInterfaceinterface RunnableWrapp1 extends Runnable {
	void _run();
	
	@Overridedefault void run() {
